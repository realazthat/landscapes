...
///type used to specify the length of an entire long-volume-array.
...
GLOBAL_STATIC_CONST uint32_t morton256_x[256] =
{
    0x00000000,
...
GLOBAL_STATIC_CONST uint32_t morton256_y[256] = {
    0x00000000,
...
GLOBAL_STATIC_CONST uint32_t morton256_z[256] = {
    0x00000000,
...
static inline uint32_t uninterleave32_3(uint32_t curve)
{
    uint32_t result = 0;
    size_t bitsize = sizeof(uint32_t)*8;
    for (size_t i = 0; i < (bitsize / 3);  ++i)
    {
        uint32_t bit = (curve >> (i*3)) & 1;
        result |= bit << i;
    }
...
static inline void vcurve2coords(vcurve_t vcurve, vside_t side, vside_t* x, vside_t* y, vside_t* z)
{
    ...
    *x = uninterleave32_3(vcurve >> 0);
    *y = uninterleave32_3(vcurve >> 1);
...
static inline ccurve_t corner2ccurve(corner_t corner)
{
    return get_corner_index(corner);
}

static inline corner_t ccurve2corner(ccurve_t ccurve)
{
    assert(ccurve < 8);
    return get_corner_by_index(ccurve);
}
...
static inline vcurvesize_t vcurvesize(vside_t side)
{
    LOCAL_STATIC_CONST vcurvesize_t vcurvesizemax = (vcurvesize_t)(-1);
    LOCAL_STATIC_CONST vcurvesize_t vcurvesidemax = vcurvesizemax/3;

    assert(side <= vcurvesidemax);

    vcurvesize_t side_ = (vcurvesize_t)(side);
...
static inline void vcurve2coords(vcurve_t vcurve, vside_t side, vside_t* x, vside_t* y, vside_t* z)
{
    ...
    vcurve_t side2 = side*side;
    vcurve_t _z = vcurve / side2;
    vcurve_t zside2 = _z*side2;

    vcurve_t _y = vcurve - (zside2);
    vcurve_t _x = vcurve - (_y*side) - (zside2);
